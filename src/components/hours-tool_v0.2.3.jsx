...
export default function App() {
  ...
  const [selectedServiceKey, setSelectedServiceKey] = useState("9th_hour");
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#FAF6EE", fontFamily: "Georgia, serif", color: "#1C1008" }}>
      ...
      <div style={{ maxWidth: "720px", margin: "0 auto", padding: "0 2rem 2rem",
        ...
        {showHowItWorks && (
          <div style={{ marginTop: "1rem", fontSize: "0.85rem", lineHeight: "1.65",
                        color: "#2C1F0A", borderLeft: "3px solid #C4A84A",
                        paddingLeft: "1rem", textAlign: "left" }}>

            <div style={{ fontWeight: "bold", fontSize: "0.75rem", marginBottom: "0.75rem",
                          color: "#5C4A1E", letterSpacing: "0.05em", textTransform: "uppercase" }}>
              The Calendar Engine
            </div>

            <p style={{ margin: "0 0 0.75rem" }}>
              The tool computes every movable feast, fast, and liturgical season from a single anchor: <strong>Pascha</strong> (Orthodox Easter).
              Pascha is calculated using the <em>Meeus/Jones/Butcher</em> algorithm applied to the Julian calendar,
              then converted to the Gregorian (Revised Julian) calendar by adding 13 days — the current difference
              between the two calendars, fixed until 2100.
            </p>

            <p style={{ margin: "0 0 0.75rem" }}>
              From Pascha, every other movable date is a simple offset. Lent begins 48 days before Pascha (Clean Monday).
              Pentecost falls 49 days after. All Saints Sunday is 56 days after. The five Lenten Sundays,
              the pre-Lenten period (Meatfare, Cheesefare), Ascension, and the Sunday of All Saints of North America
              are all computed the same way — as a signed number of days from Pascha. The algorithm has been
              verified correct against the OCA desk calendar for 2026, 2027, and 2028.
            </p>

            <p style={{ margin: "0 0 0.75rem" }}>
              The <strong>tone cycle</strong> (Tones 1–8 of the Octoechos) begins on the Monday after All Saints Sunday
              and advances by one each week, cycling continuously through ordinary time.
            </p>

            <p style={{ margin: "0 0 0.75rem" }}>
              The tool recognizes <strong>35 named movable days</strong> — from the Sunday of the Publican and Pharisee
              (Pascha−70) through All Saints of North America (Pascha+63) — and displays contextual notes for each.
              Great Feasts with their forefeasts, afterfeasts, and apodoses are tracked and influence
              which Fekula assembly rule applies.
            </p>

            <p style={{ margin: "0 0 1rem", fontStyle: "italic", color: "#5C4A1E" }}>
              Limitations: Lenten Hours (§3A), Pentecostarion weekdays (§4A), Great Feast assembly
              (§2G1–§2G4), Saturday rules, and services before the 9th Hour are not yet assembled —
              flagged in the UI when encountered.
            </p>
            ...
            <p style={{ margin: "0 0 0.75rem" }}>
              The daily scripture readings shown in the context card come from a static table of{" "}
              <strong>298 entries</strong>, each keyed by its Pascha offset (days after Pascha).
              The table was built from the OCA lectionary and verified across two years: the same
              Pascha offset yields the same readings in every year — the entire New Testament cycle
              is purely movable, anchored to Pascha, not to the calendar date.
            </p>

            <p style={{ margin: "0 0 0.75rem" }}>
              Readings begin with Acts and John at Pascha, transition to the Epistles and Matthew
              after Pentecost, and continue through the Pauline letters into autumn.
              One complication is the <strong>Lukan Jump</strong>: on the Monday after the Sunday
              on or after the Elevation of the Holy Cross (September 14), the Gospel abruptly
              leaves Matthew and begins Luke from the start — regardless of where Matthew was.
              The Epistle cycle continues uninterrupted. Because September 14 falls on a different
              day of the week each year, the exact Pascha offset where Luke begins shifts annually
              (typically P+134 to P+162) and is computed dynamically. Dates in the Luke series
              are marked with a small badge.
            </p>

            <p style={{ margin: "0 0 1rem", fontStyle: "italic", color: "#5C4A1E" }}>
              Limitations: readings are references only (e.g., "Romans 4:4-12") — scripture text
              is not yet displayed. Feast proper readings are encoded for June where data
              was available; other months are added during Menaion encoding.
              Old Testament Vespers lessons (<em>paroemias</em> — three OT readings at Vespers for
              Polyeleos-rank feasts and above) are present in the Menaion PDFs but not yet
              extracted or displayed (FW-21). If you encounter a day where the OCA website shows
              Genesis, Ezekiel, Proverbs or other OT readings that are not in this tool,
              those are Vespers paroemias — coming in a future update.
              Lenten weekday pericopes and the Slavic pericope numbering system are also planned.
            </p>

            <p style={{ margin: "0 0 1rem" }}>
              The tool currently shows up to two lines of readings: the daily cycle reading (Epistle
              and Gospel from the Paschal sequence) and, when available, the feast proper reading
              (Epistle and Gospel specific to the saint or feast). The OCA website may show
              additional readings — Vespers paroemias, Matins Gospel, or multiple feast layers —
              that are not yet represented here.
            </p>
            ...
            <p style={{ margin: "0 0 0.75rem" }}>
              Every movable element in the assembled service is governed by{" "}
              <em>Fekula & Williams, The Order of Divine Services</em> (2009),
              the standard rubrical guide for OCA parishes using the Russian usage.
              Each element carries a <span style={{ color: "#8B6914" }}>§</span> citation badge
              identifying the exact section that governs its placement.
              The fixed skeleton of each Hour comes from the <em>HTM Unabbreviated Horologion</em>;
              movable elements come from the <em>St. Sergius Parish Menaion</em>,
              with OCA calendar sources taking precedence when they diverge.
            </p>

            <p style={{ margin: "0", fontStyle: "italic", color: "#5C4A1E" }}>
              The Menaion library currently covers June 1–30. July onward is in progress.
              When a date has no Menaion entry yet, the tool flags it rather than silently omitting content.
            </p>
