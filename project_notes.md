...
(clean layer confirmed: 0 non-Latin, 0 digit-zero; review file to Bill);
`theotokia.js` generated by heading-driven parse — 184 cells across the
three tables, all eight tones. Register grew to 107 pairs (43
live-byte-checked); sic register to 26 (7 live-checked).

### Findings & flags (Bill review)

- **Part1↔Part3 dismissal reprints:** byte-identical in tones
  1/2/3/5/6/7/8; **Tone IV diverges by one capital** ("ranks of
  angels/Angels") — variant entry. 34 further identical recombinations
  WITHIN Part 3 (Lauds rows repeat across days per tone) all registered.
- **Tone VII doubled `**`:** the dismissal theotokion prints two `**`
  markers, identically at BOTH print sites (the "O Christ,?" true-dup
  class). Kept verbatim; gate exempts sic-registered `pointing_anomaly`
  positions — new register field, flagged for Bill.
- **Tone IV "Saturday Matins" heading** at the Lauds-position row (all
  other tones: "Saturday Lauds") — keyed positionally as `sat_lauds`,
  printed heading in sourceLabel + sic register. This is the row carrying
  the cross-tone "un-burnt bush" trap text (with tue_lauds).
- **Other sics:** unclosed "[Note:" bracket (Part 3 usage note); "dist
  give birth" ×2; "Tuesday-Vespers" hyphen heading (Tone VI). The 5 scan
  candidates were all house-style "...," ellipses — noise, not sics.
- Part 2 Tone II Tue-praises prints UNPOINTED (Tier 1) beside the pointed
  Sun-eve print of the same hymn — the spec's internal-variance case,
  captured automatically by the per-cell tier rule.
