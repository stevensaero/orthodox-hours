...
# 2-1…2-7 -layout text layers. Merges into the existing tone2.js (step-4
...
# output by reading the JSON payload of tone2.js).
import json, re, sys
...
FILES = [
  {'f': '2-2', 'eve': 'sun', 'night': 'sun', 'morn': 'mon'},
  {'f': '2-3', 'eve': 'mon', 'night': 'mon', 'morn': 'tue'},
  {'f': '2-4', 'eve': 'tue', 'night': 'tue', 'morn': 'wed'},
  {'f': '2-5', 'eve': 'wed', 'night': 'wed', 'morn': 'thu'},
  {'f': '2-6', 'eve': 'thu', 'night': 'thu', 'morn': 'fri'},
  {'f': '2-7', 'eve': 'fri', 'night': 'fri', 'morn': 'sat'},
]
...
def hfind(paras, frag, start=0, kind=None):
    for i in range(start, len(paras)):
        if frag in paras[i]['text'] and (kind is None or paras[i]['kind'] == kind):
            return i
...
for cfg in FILES:
    ...
    if paras[i]['text'].startswith('Spec. Mel.'):
        rubric += ' ' + paras[i]['text']
        spec_mel = re.sub(r'^Spec\. Mel\.: “(.+?)( \.\.\.)?”:$', r'\1', paras[i]['text']); i += 1
    if eve != 'fri':
        ...
        while True:
            ...
            if is_incipit(body) and n_seen:
                oct_items.append(node(body, f'Friday-evening Vespers, LIC sticheron {n_seen+1} (incipit repeat of sticheron 1, §2.7)',
                                      label='plain', incipit_ref='tone2.vespers_weekday.fri.lic.octoechos[0]',
    ...
    while True:
        ...
        if t.startswith('Verse:'): i += 1; continue   # print sites live in shared
        lab, text, printed = label_split(t)
    ...
    if morn != 'sat':
        ...
        while len(sets) < 3:
            ...
            while True:
                ...
                kw = {}
                if inc:
                    kw['incipit_ref'] = f'tone2.matins_weekday.{morn}.sessionals[{len(sets)}].items[0]'
            ...
            if t.startswith('Irmos:'):
                ...
                if is_incipit(body) and target == 2:
                    store[okey]['irmos'] = node(body, f'{DAYN[morn]} Matins, canon 2, Ode {ode} irmos (incipit device, §2.7)',
                        incipit_ref=f'tone2.matins_weekday.{morn}.canons[0].odes.{ode}.irmos')
                else:
...
paras1 = tokenize('/tmp/scan/2-1L.txt')
node1 = make_node('2-1.pdf')
...
# merge into tone2.js
src = open('/tmp/oh2/src/data/octoechos_v2/tone2.js').read()
payload = src[src.index('export default ') + len('export default '):].rstrip().rstrip(';')
tone2 = json.loads(payload)
tone2['vespers_weekday'] = vespers_weekday
tone2['compline'] = compline
tone2['matins_weekday'] = matins_weekday
tone2['liturgy_weekday'] = liturgy_weekday
claims = [c for c in tone2['_encoded'] if '.' not in c]
...
tone2['_encoded'] = claims
header = src[:src.index('export default ') + len('export default ')]
open('/tmp/oh2/src/data/octoechos_v2/tone2.js', 'w').write(header + json.dumps(tone2, ensure_ascii=False, indent=2) + ';\n')
print('merged; nodes now:', json.dumps(tone2).count('"src"'))
