# tone2.js generator — §11 step 4: core + LV + GV + Nocturns + Sunday Matins +
# Sunday Liturgy from 2-1.pdf's -layout text (paragraph-indent tokenization).
# Nothing hand-retyped. 2-1 layer is CLEAN (scan July 7 2026) — asserted.
import json, re, sys

RAW = open('/tmp/scan/2-1L.txt').read().replace('\x0c', '\n')
assert 'О' not in RAW, '2-1 must be clean'
lines = RAW.split('\n')
...
for l in lines:
    ...
    if indent >= 2 or stripped.startswith('Irmos:'):
        cur = {'kind': 'para', 'text': stripped}; paras.append(cur); continue
    if cur is None or re.match(r'^((The )?Verse:|Refrain: |Alleluia, in Tone|Irmos: |Communion Verse: |Glory \.\.\.,|Both now \.\.\.,|Theotokion: |Trinitarion: )', stripped):
...
i = find('The Resurrection Stichera, in Tone II:', start=i) + 1
lv_verses, lv_lic = [], []
for n in range(4):
    assert paras[i]['text'].startswith('Verse:'), paras[i]['text']
    lv_verses.append(node(strip_verse(paras[i]['text']), f'Little Vespers, LIC verse {n+1}')); i += 1
    lv_lic.append(node(paras[i]['text'], f'Little Vespers, LIC sticheron position {n+1}')); i += 1
assert lv_lic[0]['text'] == lv_lic[1]['text'], 'LV s1 double print must byte-match (§9.4 full-double-print device)'
...
lv_theo_stich = []
for n in range(3):
    assert paras[i]['text'].startswith('Verse:'), paras[i]['text']; i += 1   # verses = shared print site
...
anatolius = False
for n in range(7):
    if paras[i]['text'] == 'Other Stichera, by Anatolius:' or (paras[i]['kind']=='heading' and 'Anatolius' in paras[i]['text']):
        anatolius = True; i += 1
    assert paras[i]['text'].startswith('Verse:'), (n, paras[i]['text'])
    gv_verses.append(node(strip_verse(paras[i]['text']), f'Great Vespers, LIC ladder verse {n+1}')); i += 1
    extra = {'provenance_note': 'sub-group "Other Stichera, by Anatolius" (§4.3)'} if anatolius else {}
    gv_lic.append(node(paras[i]['text'], f'Great Vespers, LIC sticheron {n+1}', **extra)); i += 1
...
i = find('On the Aposticha, these Stichera, in Tone II:', start=i) + 1
gv_ap = [node(paras[i]['text'], 'Great Vespers, aposticha sticheron 1 (unversed)')]; i += 1
for n in range(3):
    assert paras[i]['text'].startswith('Verse:'), paras[i]['text']; i += 1  # shared print site
...
assert paras[i]['text'] == 'Tone IV:'; i += 1
...
lit_i = find('AT LITURGY', 'heading')
lit_trop_i = find('Resurrection Troparion, in Tone II:', start=lit_i) + 1
...
for k, s in sites.items():
    assert normquote(s) == base, f'troparion site {k} diverges beyond the recorded §9.5 variance:\n{normquote(s)}\n{base}'
assert '“O Life-giver' in lv_troparion_site and '“' not in gv_troparion_site
troparion = node(gv_troparion_site, 'Great Vespers, if-no-Vigil (CANONICAL print, §9.5)',
...
kont_i = find('Resurrection Kontakion, in Tone II:', start=find('AT MATINS','heading')) + 1
...
lit_kont_i = find('Resurrection Kontakion, in Tone II:', start=lit_i) + 1
assert paras[lit_kont_i]['text'] == kontakion_site1, 'kontakion sites must byte-match (§4.1)'
kontakion = node(kontakion_site1, 'Sunday Matins after Ode VI (verified identical at the Liturgy site)')
...
def parse_shape_b(start_i, end_i, where):
    ...
    while j < end_i:
        ...
        if t.startswith('Sessional Hymn'):
            ...
            spec_mel = None; j += 1
            if paras[j]['text'].startswith('Spec. Mel.'):
                spec_mel = re.sub(r'^Spec\. Mel\.: “(.+)”:$', r'\1', paras[j]['text']); j += 1
...
m = re.search(r'the acrostic whereof is “(.+?),?” the composition of (.+?), in Tone II', head)
assert m, head
...
for o in (6, 7, 8):
    assert noc_odes[o]['items'][0].get('repeat') == 2, f'Nocturns Ode {o} first item must carry (Twice) — spec §4.6'
...
for kath, label in [(i, 'Kathisma II'), (find('After the 2nd chanting of the Psalter'), 'Kathisma III')]:
    ...
    assert paras[j]['text'].startswith('Glory ..., Both now ...'), paras[j]['text']; j += 1
...
assert sessionals[0]['closer']['type'] == 'stavrotheotokion' and sessionals[1]['closer']['type'] == 'theotokion'

hyp_i = find('The Sessional Hymn:', 'heading')
hypakoe = node(paras[hyp_i+1]['text'], 'Sunday Matins, after the Evlogitaria', sourceLabel='The Sessional Hymn')
asc_i = find('The Songs of Ascent: 1st Antiphon:')
anabathmoi = []
j = asc_i
for ant in range(3):
    assert 'Antiphon' in paras[j]['text']; j += 1
...
assert paras[j]['text'].startswith('Prokeimenon, in Tone II:'), paras[j]['text']
m_prok = node(re.sub(r'^Prokeimenon, in Tone II: ', '', paras[j]['text']), 'Sunday Matins prokeimenon'); j += 1
assert paras[j]['text'].startswith('The Verse:')
...
def parse_shape_a(start_i, end_i):
    ...
    while j < end_i:
        ...
        if t.startswith('Resurrection Kontakion') or t.startswith('Ikos:'):
            ...
            continue
        m3 = re.match(r'^(Theotokion|Trinitarion): ', t)
        if m3:
            cur['closer'] = node(t[m3.end():], f'Sunday Matins canon, Ode {ode}, {sub} closer',
                                 type=m3.group(1).lower(), refrain=pending_refrain, sourceLabel=m3.group(1))
            pending_refrain = None; j += 1; continue
        # plain troparion
...
canon_start = find('Resurrection canon, in Tone II:', 'heading')
...
for o, od in odes_a.items():
    ...
    want = 'trinitarion' if o == 9 else 'theotokion'
    assert od['resurrection'].get('closer', {}).get('type') == want, (o, od['resurrection'].get('closer'))
...
pr_stichera = []
for n in range(8):
    if 'Anatolius' in paras[j]['text']: j += 1
    assert paras[j]['text'].startswith('Verse:'), paras[j]['text']; j += 1   # shared ladder print site
    pr_stichera.append(node(paras[j]['text'], f'Sunday Matins, Praises sticheron {n+1}',
        **({'provenance_note': 'sub-group "Other Stichera of Anatolius"'} if n >= 4 else {}))); j += 1
...
matins = {
  ...
  'prokeimenon': {'tone': 2, 'text': m_prok, 'verse': m_prok_verse},
  'canon': {'title': 'Resurrection canon', 'heading_rubric': 'After which: “O God, save Thy people ...,” Then the Canons: Resurrection canon, in Tone II:',
...
# ═══ SUNDAY LITURGY (§4.9) ════════════════════════════════════════════════════
j = lit_i + 1
assert paras[j]['text'] == 'Typika and Beatitudes.'; beat_rubric = paras[j]['text']; j += 1
beat = []
for n in range(6):
    beat.append(node(paras[j]['text'], f'Sunday Liturgy, Beatitude troparion {n+1}')); j += 1
gloria = node(paras[j]['text'], 'Sunday Liturgy, Beatitudes Gloria (Triadicon)'); j += 1
...
j = find('The Prokeimenon, in Tone II:', start=lit_i)
l_prok = node(re.sub(r'^The Prokeimenon, in Tone II: ', '', paras[j]['text']), 'Sunday Liturgy prokeimenon'); j += 1
assert paras[j]['text'].startswith('Verse:')
l_prok_verse = node(strip_verse(paras[j]['text']), 'Sunday Liturgy prokeimenon verse'); j += 1
assert paras[j]['text'].startswith('Alleluia, in Tone II:')
l_all = node(re.sub(r'^Alleluia, in Tone II: ', '', paras[j]['text']), 'Sunday Liturgy Alleluia'); j += 1
assert paras[j]['text'].startswith('Verse:')
...
liturgy = {
  ...
  'prokeimenon': {'tone': 2, 'text': l_prok, 'verse': l_prok_verse},
  'alleluia': {'tone': 2, 'verses': [l_all, l_all_v]},
}
...
header = '''// src/data/octoechos_v2/tone2.js
// ─────────────────────────────────────────────────────────────────────────────
...
export default '''
open('/tmp/oh2/src/data/octoechos_v2/tone2.js', 'w').write(header + json.dumps(OUT, ensure_ascii=False, indent=2) + ';\n')
n_nodes = json.dumps(OUT).count('"src"')
print(f'tone2.js written — {n_nodes} text nodes')
