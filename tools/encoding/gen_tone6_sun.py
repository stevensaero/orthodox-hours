...
RAW = open('/tmp/scan6/6-1L.txt').read().replace('\x0c', '')   # 4-1 print fact: 3 page breaks fall MID-PARAGRAPH — formfeed is a page separator, not a paragraph boundary
...
i = find('The Resurrection Stichera, in Tone VI', start=i)   # 5-1 prints 'Tone VI.' (period, not colon) — heading-punctuation print fact
if paras[i]['text'].endswith('.'):
    findings.append("LV stichera heading prints 'The Resurrection Stichera, in Tone VI.' — period for colon (heading sic class)")
i += 1
lv_verses, lv_lic = [], []
while paras[i]['text'].startswith('Verse:'):
    ...
    if _lt.startswith('Repeat: '):
        ...
        lv_lic.append(node(_lt[len('Repeat: '):], f'Little Vespers, LIC sticheron position {len(lv_lic)+1} (incipit repeat of position 1, §2.7, printed with an explicit "Repeat:" label)',
                           incipit_ref='tone6.little_vespers.lic[0]', sourceLabel='Repeat:'))
    else:
...
i = find('On the Aposticha, these Stichera, in Tone VI:', start=i) + 1
...
for k, s in sites.items():
    ...
    if k == 'LV' and ns == LV_BE_DROP and LV_BE_DROP != base:
        ...
        continue
    raise AssertionError(f'troparion site {k} diverges beyond the known tone-6 variant:\n{ns}\n{base}')
quoted = [k for k, s in sites.items() if '“' in s]
findings.append(f'troparion print sites: {len(sites)} checked; quote-marked sites: {quoted or "none"} (tone 2: LV only); word-level site divergences: {site_divergences or "none"}')
if site_divergences:
    findings.append('SIC CANDIDATE (6-1 LV troparion): "glory to Thee" drops "be" vs the other three sites\' "glory be to Thee" — dropped-word class, encode-as-printed, GV canonical')
troparion = node(gv_troparion_site, 'Great Vespers, if-no-Vigil (CANONICAL print, §9.5 convention)',
  provenance_note=f'Verified pointing-identical at all four print sites (LV dismissal, GV no-vigil, Matins God-is-the-Lord, Liturgy); quotation-mark variance at: {", ".join(quoted) if quoted else "none"}; word-level divergence: {site_divergences.get("LV", "none")}. Canonical field stores the GV print per the §9.5 ruling.')
...
kont_i = find('Resurrection Kontakion, in Tone VI:', start=find('AT MATINS','heading')) + 1
kont_sm = None
...
# prints 'Kontakion of the Resurrection, in Tone VI:' (word-order variant),
# while the Matins site prints 'Resurrection Kontakion, in Tone VI:'. Text is
# byte-identical; only the heading differs. Accept either KNOWN form exactly.
LIT_KONT_HEADINGS = ('Resurrection Kontakion, in Tone VI:', 'Kontakion of the Resurrection, in Tone VI:')
...
if _lit_kont_head != 'Resurrection Kontakion, in Tone VI:':
    findings.append(f'Liturgy kontakion heading prints {_lit_kont_head!r} (Matins site: \'Resurrection Kontakion, in Tone VI:\') — per-print heading word-order variance (per-tone print fact)')
...
# (acrostic on its own line; 'Tone VI:' centered) AND prints 'Tone VI:' WITHOUT
# 'in' (5-1 printed a single-line heading with 'in Tone V'). Join forward to the
# tone-line terminator; make 'in' optional. Backward-compatible: if 'Tone VI' is
...
while 'Tone VI' not in head and canon_body_start + 1 < len(paras):
    canon_body_start += 1; head += ' ' + paras[canon_body_start]['text']
head = re.sub(r'\s+', ' ', head).strip()
m = re.search(r'the acrostic whereof is “(.+?),?” the composition of (.+?),? (?:in )?Tone VI', head)
assert m, head
...
hyp_i = find('The Sessional Hymn', 'heading')   # 5-1 prints 'The Sessional Hymn, in Tone VI:' (3-1/4-1: 'The Sessional Hymn:')
...
assert paras[j]['text'].startswith('Prokeimenon, in Tone VI:'), paras[j]['text']
m_prok = node(re.sub(r'^Prokeimenon, in Tone VI: ', '', paras[j]['text']), 'Sunday Matins prokeimenon'); j += 1
m_prok_verse = None
...
j = find('The Prokeimenon, in Tone VI:', start=lit_i)
l_prok = node(re.sub(r'^The Prokeimenon, in Tone VI: ', '', paras[j]['text']), 'Sunday Liturgy prokeimenon'); j += 1
...
assert paras[j]['text'].startswith('Alleluia, in Tone VI:')
l_all = node(re.sub(r'^Alleluia, in Tone VI: ', '', paras[j]['text']), 'Sunday Liturgy Alleluia'); j += 1
assert paras[j]['text'].startswith(('Verse:', 'The Verse:'))
...
header = '''// src/data/octoechos_v2/tone6.js
// ─────────────────────────────────────────────────────────────────────────────
...
export default '''
open('/tmp/oh/src/data/octoechos_v2/tone6.js', 'w').write(header + json.dumps(OUT, ensure_ascii=False, indent=2) + ';\n')
n_nodes = json.dumps(OUT).count('"src"')
print(f'tone6.js written — {n_nodes} text nodes')
import pickle
pickle.dump(CAPTURED, open('/tmp/gen6/captured_t6_sun.pkl','wb'))
