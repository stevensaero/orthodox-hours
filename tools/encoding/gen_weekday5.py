...
FILES = [
  {'f': '5-2', 'eve': 'sun', 'night': 'sun', 'morn': 'mon'},
  {'f': '5-3', 'eve': 'mon', 'night': 'mon', 'morn': 'tue'},
  {'f': '5-4', 'eve': 'tue', 'night': 'tue', 'morn': 'wed'},
  {'f': '5-5', 'eve': 'wed', 'night': 'wed', 'morn': 'thu'},
  {'f': '5-6', 'eve': 'thu', 'night': 'thu', 'morn': 'fri'},
  {'f': '5-7', 'eve': 'fri', 'night': 'fri', 'morn': 'sat'},
]
...
for cfg in FILES:
    f = cfg['f']; fname = f + '.pdf'
    paras = tokenize(f'/tmp/scan5/{f}L.txt')
        ...
        while True:
            ...
            if is_incipit(body) and n_seen:
                ...
                oct_items.append(node(body, f'Friday-evening Vespers, LIC sticheron {n_seen+1} (incipit repeat of sticheron {_refs[0]+1}, §2.7' + (', printed with an explicit "Repeat:" label' if _rep_label else '') + ')',
                                      label='plain', incipit_ref=f'tone5.vespers_weekday.fri.lic.octoechos[{_refs[0]}]',
    ...
    if morn != 'sat':
        ...
        while len(sets) < 3:
            ...
            while True:
                ...
                kw = {}
                if inc:
                    kw['incipit_ref'] = f'tone5.matins_weekday.{morn}.sessionals[{len(sets)}].items[0]'
            ...
            if t.startswith('Irmos:'):
                ...
                if is_incipit(body) and target == 2:
                    store[okey]['irmos'] = node(body, f'{DAYN[morn]} Matins, canon 2, Ode {ode} irmos (incipit device, §2.7)',
                        incipit_ref=f'tone5.matins_weekday.{morn}.canons[0].odes.{ode}.irmos')
                else:
...
paras1 = tokenize('/tmp/scan5/5-1L.txt')
node1 = make_node('5-1.pdf')
...
# merge into tone3.js
src = open('/tmp/oh/src/data/octoechos_v2/tone5.js').read()
payload = src[src.index('export default ') + len('export default '):].rstrip().rstrip(';')
tone5 = json.loads(payload)
tone5['vespers_weekday'] = vespers_weekday
tone5['compline'] = compline
tone5['matins_weekday'] = matins_weekday
tone5['liturgy_weekday'] = liturgy_weekday
claims = [c for c in tone5['_encoded'] if '.' not in c]
...
tone5['_encoded'] = claims
header = src[:src.index('export default ') + len('export default ')]
open('/tmp/oh/src/data/octoechos_v2/tone5.js', 'w').write(header + json.dumps(tone5, ensure_ascii=False, indent=2) + ';\n')
import pickle
pickle.dump(CAPTURED_WK, open('/tmp/gen5/captured_t5_wk.pkl','wb'))
print('merged; nodes now:', json.dumps(tone5).count('"src"'))
