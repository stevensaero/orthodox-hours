...
RAW = open('/tmp/scan4/4-1L.txt').read().replace('\x0c', '')   # 4-1 print fact: 3 page breaks fall MID-PARAGRAPH — formfeed is a page separator, not a paragraph boundary
...
i = find('The Resurrection Stichera, in Tone IV:', start=i) + 1
lv_verses, lv_lic = [], []
...
i = find('On the Aposticha, these Stichera, in Tone IV:', start=i) + 1
...
kont_i = find('Resurrection Kontakion, in Tone IV:', start=find('AT MATINS','heading')) + 1
kont_sm = None
...
lit_kont_i = find('Resurrection Kontakion, in Tone IV:', start=lit_i) + 1
...
m = re.search(r'the acrostic whereof is “(.+?),?” the composition of (.+?), in Tone IV', head)
assert m, head
...
assert paras[j]['text'].startswith('Prokeimenon, in Tone IV:'), paras[j]['text']
m_prok = node(re.sub(r'^Prokeimenon, in Tone IV: ', '', paras[j]['text']), 'Sunday Matins prokeimenon'); j += 1
m_prok_verse = None
...
findings.append('Shape A sub-canon headings: Ode I prints \'Another Canon, of Cross and Resurrection:\' / \'Another Canon, to the most holy Theotokos:\'; Odes III-IX print the short forms — per-print heading variance within 4-1')
closer_census = {}
...
matins = {
  ...
  'prokeimenon': {'tone': 4, 'text': m_prok, **({'verse': m_prok_verse} if m_prok_verse else {})},
...
j = find('The Prokeimenon, in Tone IV:', start=lit_i)
l_prok = node(re.sub(r'^The Prokeimenon, in Tone IV: ', '', paras[j]['text']), 'Sunday Liturgy prokeimenon'); j += 1
...
assert paras[j]['text'].startswith('Alleluia, in Tone IV:')
l_all = node(re.sub(r'^Alleluia, in Tone IV: ', '', paras[j]['text']), 'Sunday Liturgy Alleluia'); j += 1
assert paras[j]['text'].startswith(('Verse:', 'The Verse:'))
...
liturgy = {
  ...
  'prokeimenon': {'tone': 4, 'text': l_prok, 'verse': l_prok_verse},
  'alleluia': {'tone': 4, 'verses': [l_all, l_all_v]},
}
...
header = '''// src/data/octoechos_v2/tone4.js
// ─────────────────────────────────────────────────────────────────────────────
...
export default '''
open('/tmp/oh/src/data/octoechos_v2/tone4.js', 'w').write(header + json.dumps(OUT, ensure_ascii=False, indent=2) + ';\n')
n_nodes = json.dumps(OUT).count('"src"')
print(f'tone4.js written — {n_nodes} text nodes')
import pickle
pickle.dump(CAPTURED, open('/tmp/gen4/captured_t4_sun.pkl','wb'))
