...
RAW = open('/tmp/scan7/7-1L.txt').read().replace('\x0c', '')   # 4-1 print fact: 3 page breaks fall MID-PARAGRAPH — formfeed is a page separator, not a paragraph boundary
...
# a following paragraph that itself OPENS a structural unit must never be
# absorbed into a preceding non-terminal verse/refrain (tone-7 finding: the
# Ode-IX 'Refrain: We bless the Lord; Father, Son, and Holy Spirit' has no
# terminal punctuation and was gluing the following 'Trinitarian:' closer onto
# itself, silently dropping the closer).
...
i = find('The Resurrection Stichera, in Tone VII', start=i)   # 5-1 prints 'Tone VII.' (period, not colon) — heading-punctuation print fact
if paras[i]['text'].endswith('.'):
    findings.append("LV stichera heading prints 'The Resurrection Stichera, in Tone VII.' — period for colon (heading sic class)")
i += 1
lv_verses, lv_lic = [], []
while paras[i]['text'].startswith('Verse:'):
    ...
    if _lt.startswith('Repeat: '):
        ...
        lv_lic.append(node(_lt[len('Repeat: '):], f'Little Vespers, LIC sticheron position {len(lv_lic)+1} (incipit repeat of position 1, §2.7, printed with an explicit "Repeat:" label)',
                           incipit_ref='tone7.little_vespers.lic[0]', sourceLabel='Repeat:'))
    else:
...
i = find('On the Aposticha, these Stichera, in Tone VII:', start=i) + 1
...
# prints 'Resurrection Theotokion, in Tone VII:' WITHOUT 'the' (tone 6 printed
...
if 'the Resurrection Theotokion' not in paras[i]['text']:
    findings.append("GV dismissal-theotokion heading prints %r (no 'the'; tone 6: 'the Resurrection Theotokion') — per-print heading fact" % paras[i]['text'])
i += 1
...
gil_rubric = paras[i]['text']; assert gil_rubric.startswith(('On “God is The Lord', 'At “God is The Lord')), gil_rubric   # tone-7: 'At' for 'On' (per-print preposition variant)
if gil_rubric.startswith('At '): findings.append("God-is-the-Lord rubric prints 'At \u201cGod is The Lord \u2026\u201d' ('At' for 'On') — per-print heading fact")
i += 1
...
# EXACT-variant guard (tone-6 finding, 6-1): the LITTLE VESPERS troparion print
# drops the word 'be' — 'glory to Thee' for the GV/Matins/Liturgy 'glory be to
# Thee'. Word-level per-site divergence in a canonical §4.1 field; GV stays
# canonical (§9.5), the LV drop is recorded in provenance + flagged a candidate
# sic (dropped-word class). Guard is EXACT — any OTHER divergence still hard-fails.
LV_BE_DROP = base.replace('glory be to Thee', 'glory to Thee')
def _normpoint(t): return t.replace('**', '*')   # collapse pointing for WORD-level equality (marks compared separately)
base_w = _normpoint(base)
site_divergences = {}
pointing_divergences = {}
for k, s in sites.items():
    ns = normquote(s)
    if ns == base:
        continue
    if _normpoint(ns) == base_w:
        # WORDS identical; the only difference is pointing (* vs **) — a per-site
        # pointing-provenance variance (tone-7 finding: Matins God-is-the-Lord
        # prints a single * where GV prints ** at the penultimate mark). GV stays
        # canonical (§9.5); recorded, not lost. Word check above stays EXACT.
        pointing_divergences[k] = 'single */double ** pointing differs from the GV print at a mark'
        continue
    if k == 'LV' and ns == LV_BE_DROP and LV_BE_DROP != base:
        site_divergences['LV'] = 'prints "glory to Thee" — drops "be" vs the GV/Matins/Liturgy "glory be to Thee" (candidate sic, dropped word)'
        continue
    raise AssertionError(f'troparion site {k} diverges in WORDS beyond the known variants:\n{ns}\n{base}')
quoted = [k for k, s in sites.items() if '“' in s]
findings.append(f'troparion print sites: {len(sites)} checked; quote-marked sites: {quoted or "none"} (tone 2: LV only); word-level divergences: {site_divergences or "none"}; pointing divergences: {pointing_divergences or "none"}')
if site_divergences:
...
kont_i = find('Resurrection Kontakion, in Tone VII:', start=find('AT MATINS','heading')) + 1
kont_sm = None
...
# prints 'Kontakion of the Resurrection, in Tone VII:' (word-order variant),
# while the Matins site prints 'Resurrection Kontakion, in Tone VII:'. Text is
# byte-identical; only the heading differs. Accept either KNOWN form exactly.
LIT_KONT_HEADINGS = ('Resurrection Kontakion, in Tone VII:', 'Kontakion of the Resurrection, in Tone VII:')
lit_kont_i = None; _lit_kont_head = None
for _h in LIT_KONT_HEADINGS:
    try:
        lit_kont_i = find(_h, start=lit_i) + 1; _lit_kont_head = _h; break
    except AssertionError:
        continue
assert lit_kont_i is not None, 'Liturgy kontakion heading not found in any known variant'
if _lit_kont_head != 'Resurrection Kontakion, in Tone VII:':
    findings.append(f'Liturgy kontakion heading prints {_lit_kont_head!r} (Matins site: \'Resurrection Kontakion, in Tone VII:\') — per-print heading word-order variance (per-tone print fact)')
...
# (acrostic on its own line; 'Tone VII:' centered) AND prints 'Tone VII:' WITHOUT
# 'in' (5-1 printed a single-line heading with 'in Tone V'). Join forward to the
# tone-line terminator; make 'in' optional. Backward-compatible: if 'Tone VII' is
...
while 'Tone VII' not in head and canon_body_start + 1 < len(paras):
    canon_body_start += 1; head += ' ' + paras[canon_body_start]['text']
head = re.sub(r'\s+', ' ', head).strip()
m = re.search(r'the acrostic whereof is “(.+?),?” the composition of (.+?),? (?:in )?Tone VII', head)
assert m, head
...
hyp_i = find('The Sessional Hymn', 'heading')   # 5-1 prints 'The Sessional Hymn, in Tone VII:' (3-1/4-1: 'The Sessional Hymn:')
...
assert paras[j]['text'].startswith('Prokeimenon, in Tone VII:'), paras[j]['text']
m_prok = node(re.sub(r'^Prokeimenon, in Tone VII: ', '', paras[j]['text']), 'Sunday Matins prokeimenon'); j += 1
m_prok_verse = None
...
j = find('The Prokeimenon, in Tone VII:', start=lit_i)
l_prok = node(re.sub(r'^The Prokeimenon, in Tone VII: ', '', paras[j]['text']), 'Sunday Liturgy prokeimenon'); j += 1
...
assert paras[j]['text'].startswith('Alleluia, in Tone VII:')
l_all = node(re.sub(r'^Alleluia, in Tone VII: ', '', paras[j]['text']), 'Sunday Liturgy Alleluia'); j += 1
assert paras[j]['text'].startswith(('Verse:', 'The Verse:'))
...
header = '''// src/data/octoechos_v2/tone7.js
// ─────────────────────────────────────────────────────────────────────────────
...
export default '''
open('/tmp/oh/src/data/octoechos_v2/tone7.js', 'w').write(header + json.dumps(OUT, ensure_ascii=False, indent=2) + ';\n')
n_nodes = json.dumps(OUT).count('"src"')
print(f'tone7.js written — {n_nodes} text nodes')
import pickle
pickle.dump(CAPTURED, open('/tmp/gen7/captured_t7_sun.pkl','wb'))
