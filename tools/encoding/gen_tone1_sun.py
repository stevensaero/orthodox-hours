...
_HG_SUN = {'О':'O','М':'M','о':'o','С':'C','а':'a'}
def node(text, locus, tier=None, **extra):
    assert text, locus
    log = []
    for cy, la in _HG_SUN.items():
        n = text.count(cy)
...
def ode_of(t):
    for _c,_r in {'О':'O','М':'M','о':'o','С':'C','а':'a'}.items(): t=t.replace(_c,_r)
...
# tone-8 finding: the virgin-rejoice heading ('Tone IV:'/'Then, in Tone IV:') is
# OMITTED — 'O Theotokos and Virgin, rejoice …' prints directly (tones 5-7 print a
# heading). Accept both; the following line is the shared 'O Theotokos' print site.
if paras[i]['text'] in ('Tone IV:', 'Then, in Tone IV:'):
    ...
    assert paras[i]['text'].startswith('O Theotokos and Virgin'), paras[i]['text']
    findings.append("virgin-rejoice printed WITHOUT a 'Tone IV:' heading (tones 5-7 print one) — per-print heading fact")
