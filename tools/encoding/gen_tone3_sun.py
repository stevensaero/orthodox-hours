...
RAW = open('/tmp/scan/3-1L.txt').read().replace('\x0c', '\n')
assert 'О' not in RAW and 'М' not in RAW, 'N-1 must be clean'
lines = RAW.split('\n')
...
def node(text, locus, tier=None, **extra):
    assert text and 'О' not in text and 'М' not in text, locus
    d = {'text': text, 'tier': tier if tier is not None else (2 if '*' in text else 1),
         'src': {'file': '3-1.pdf', 'locus': locus}}
...
i = find('The Resurrection Stichera, in Tone III:', start=i) + 1
lv_verses, lv_lic = [], []
while paras[i]['text'].startswith('Verse:'):
    ...
    lv_lic.append(node(paras[i]['text'], f'Little Vespers, LIC sticheron position {len(lv_lic)+1}')); i += 1
findings.append(f'LV LIC positions: {len(lv_lic)} (tone 2: 4)')
if lv_lic[0]['text'] == lv_lic[1]['text']:
...
i = find('On the Aposticha, these Stichera, in Tone III:', start=i) + 1
...
kont_i = find('Resurrection Kontakion, in Tone III:', start=find('AT MATINS','heading')) + 1
kont_sm = None
if paras[kont_i]['text'].startswith('Spec. Mel.'):
    kont_sm = re.sub(r'^Spec\. Mel\.: “(.+?)( \.\.\.)?”:$', r'\1', paras[kont_i]['text']); kont_i += 1
...
lit_kont_i = find('Resurrection Kontakion, in Tone III:', start=lit_i) + 1
...
m = re.search(r'the acrostic whereof is “(.+?),?” the composition of (.+?), in Tone III', head)
assert m, head
...
assert paras[j]['text'].startswith('Prokeimenon, in Tone III:'), paras[j]['text']
m_prok = node(re.sub(r'^Prokeimenon, in Tone III: ', '', paras[j]['text']), 'Sunday Matins prokeimenon'); j += 1
m_prok_verse = None
...
matins = {
  ...
  'prokeimenon': {'tone': 3, 'text': m_prok, **({'verse': m_prok_verse} if m_prok_verse else {})},
...
j = find('The Prokeimenon, in Tone III:', start=lit_i)
l_prok = node(re.sub(r'^The Prokeimenon, in Tone III: ', '', paras[j]['text']), 'Sunday Liturgy prokeimenon'); j += 1
...
assert paras[j]['text'].startswith('Alleluia, in Tone III:')
l_all = node(re.sub(r'^Alleluia, in Tone III: ', '', paras[j]['text']), 'Sunday Liturgy Alleluia'); j += 1
assert paras[j]['text'].startswith(('Verse:', 'The Verse:'))
...
liturgy = {
  ...
  'prokeimenon': {'tone': 3, 'text': l_prok, 'verse': l_prok_verse},
  'alleluia': {'tone': 3, 'verses': [l_all, l_all_v]},
}
...
header = '''// src/data/octoechos_v2/tone3.js
// ─────────────────────────────────────────────────────────────────────────────
...
export default '''
open('/tmp/oh2/src/data/octoechos_v2/tone3.js', 'w').write(header + json.dumps(OUT, ensure_ascii=False, indent=2) + ';\n')
n_nodes = json.dumps(OUT).count('"src"')
print(f'tone3.js written — {n_nodes} text nodes')
import pickle
pickle.dump(CAPTURED, open('/tmp/gen/captured_t3_sun.pkl','wb'))
