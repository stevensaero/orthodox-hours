...
FILES = [
  {'f': '6-2', 'eve': 'sun', 'night': 'sun', 'morn': 'mon'},
  {'f': '6-3', 'eve': 'mon', 'night': 'mon', 'morn': 'tue'},
  {'f': '6-4', 'eve': 'tue', 'night': 'tue', 'morn': 'wed'},
  {'f': '6-5', 'eve': 'wed', 'night': 'wed', 'morn': 'thu'},
  {'f': '6-6', 'eve': 'thu', 'night': 'thu', 'morn': 'fri'},
  {'f': '6-7', 'eve': 'fri', 'night': 'fri', 'morn': 'sat'},
]
...
for cfg in FILES:
    f = cfg['f']; fname = f + '.pdf'
    paras = tokenize(f'/tmp/scan6/{f}L.txt')
        ...
        while True:
            ...
            if is_incipit(body) and n_seen:
                ...
                oct_items.append(node(body, f'Friday-evening Vespers, LIC sticheron {n_seen+1} (incipit repeat of sticheron {_refs[0]+1}, §2.7' + (', printed with an explicit "Repeat:" label' if _rep_label else '') + ')',
                                      label='plain', incipit_ref=f'tone6.vespers_weekday.fri.lic.octoechos[{_refs[0]}]',
    ...
    if morn != 'sat':
        ...
        while len(sets) < 3:
            ...
            while True:
                ...
                kw = {}
                if inc:
                    kw['incipit_ref'] = f'tone6.matins_weekday.{morn}.sessionals[{len(sets)}].items[0]'
            ...
            if t.startswith('Irmos:'):
                ...
                if is_incipit(body) and target == 2:
                    store[okey]['irmos'] = node(body, f'{DAYN[morn]} Matins, canon 2, Ode {ode} irmos (incipit device, §2.7)',
                        incipit_ref=f'tone6.matins_weekday.{morn}.canons[0].odes.{ode}.irmos')
                else:
...
paras1 = tokenize('/tmp/scan6/6-1L.txt')
node1 = make_node('6-1.pdf')
...
# merge into tone3.js
src = open('/tmp/oh/src/data/octoechos_v2/tone6.js').read()
payload = src[src.index('export default ') + len('export default '):].rstrip().rstrip(';')
tone6 = json.loads(payload)
tone6['vespers_weekday'] = vespers_weekday
tone6['compline'] = compline
tone6['matins_weekday'] = matins_weekday
tone6['liturgy_weekday'] = liturgy_weekday
claims = [c for c in tone6['_encoded'] if '.' not in c]
...
tone6['_encoded'] = claims
header = src[:src.index('export default ') + len('export default ')]
open('/tmp/oh/src/data/octoechos_v2/tone6.js', 'w').write(header + json.dumps(tone6, ensure_ascii=False, indent=2) + ';\n')
import pickle
pickle.dump(CAPTURED_WK, open('/tmp/gen6/captured_t6_wk.pkl','wb'))
print('merged; nodes now:', json.dumps(tone6).count('"src"'))
