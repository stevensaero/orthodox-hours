...
# against the TONE-6 raw layers (file remap 2-N → 6-N). Marker misses and
...
for f in ['2-1','2-2','2-3','2-4','2-5','2-6','2-7']:
    t3 = f.replace('2-','7-')
    RAW[f] = open(f'/tmp/scan7/{t3}.txt').read().split('\n')
...
print('THURSDAY ANOMALY in tone 7:', 'REPEATED (Vespers verse at Matins position)' if thu1 else 'NOT repeated — checking normal pair')
if not thu1:
    m1 = extract('2-5', *M.M_PAIR_STD_1)
    print('  tone-7 Thursday Matins first verse normal:', bool(m1))
# departed sets
...
print(f"\n§5 WEEKDAY/SHARED verification vs tone 7: {report['match']} matched")
print(f"marker MISSES ({len(report['miss'])}):", report['miss'])
for name, sh, t3 in report['diverge']:
    print('DIVERGE', name); print('  T2/shared:', json.dumps(sh)); print('  T6       :', json.dumps(t3))
