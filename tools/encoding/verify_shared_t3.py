...
for f in ['2-1','2-2','2-3','2-4','2-5','2-6','2-7']:
    t3 = f.replace('2-','3-')
    RAW[f] = open(f'/tmp/scan/{t3}.txt').read().split('\n')
...
shared = json.loads(open('/tmp/oh2/src/data/octoechos_v2/shared.js').read().split('export default ',1)[1].rstrip().rstrip(';'))
report = {'match': 0, 'diverge': [], 'miss': []}
def cmp(name, t3, sh):
    if t3 is None:
        report['miss'].append(name); return
    a = strip_label(t3).replace('О','O').replace('М','M')
...
print('THURSDAY ANOMALY in tone 3:', 'REPEATED (Vespers verse at Matins position)' if thu1 else 'NOT repeated — checking normal pair')
if not thu1:
    m1 = extract('2-5', *M.M_PAIR_STD_1)
    print('  tone-3 Thursday Matins first verse normal:', bool(m1))
# departed sets
...
print(f"\n§5 WEEKDAY/SHARED verification vs tone 3: {report['match']} matched")
print(f"marker MISSES ({len(report['miss'])}):", report['miss'])
for name, sh, t3 in report['diverge']:
    print('DIVERGE', name); print('  T2/shared:', json.dumps(sh)); print('  T3       :', json.dumps(t3))
