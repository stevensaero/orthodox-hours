...
for f in ['2-1','2-2','2-3','2-4','2-5','2-6','2-7']:
    t3 = f.replace('2-','8-')
    RAW[f] = open(f'/tmp/scan8/{t3}.txt').read().split('\n')
...
print('THURSDAY ANOMALY in tone 8:', 'REPEATED (Vespers verse at Matins position)' if thu1 else 'NOT repeated — checking normal pair')
if not thu1:
    m1 = extract('2-5', *M.M_PAIR_STD_1)
    print('  tone-8 Thursday Matins first verse normal:', bool(m1))
# departed sets
...
print(f"\n§5 WEEKDAY/SHARED verification vs tone 8: {report['match']} matched")
