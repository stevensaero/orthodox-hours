...
FILES = [
  {'f': '8-2', 'eve': 'sun', 'night': 'sun', 'morn': 'mon'},
  {'f': '8-3', 'eve': 'mon', 'night': 'mon', 'morn': 'tue'},
  {'f': '8-4', 'eve': 'tue', 'night': 'tue', 'morn': 'wed'},
  {'f': '8-5', 'eve': 'wed', 'night': 'wed', 'morn': 'thu'},
  {'f': '8-6', 'eve': 'thu', 'night': 'thu', 'morn': 'fri'},
  {'f': '8-7', 'eve': 'fri', 'night': 'fri', 'morn': 'sat'},
]
...
for cfg in FILES:
    f = cfg['f']; fname = f + '.pdf'
    paras = tokenize(f'/tmp/scan8/{f}L.txt')
        ...
        while True:
            ...
            if is_incipit(body) and n_seen:
                ...
                oct_items.append(node(body, f'Friday-evening Vespers, LIC sticheron {n_seen+1} (incipit repeat of sticheron {_refs[0]+1}, §2.7' + (', printed with an explicit "Repeat:" label' if _rep_label else '') + ')',
                                      label='plain', incipit_ref=f'tone8.vespers_weekday.fri.lic.octoechos[{_refs[0]}]',
    ...
    if morn != 'sat':
        ...
        while len(sets) < 3:
            ...
            while True:
                ...
                kw = {}
                if inc:
                    kw['incipit_ref'] = f'tone8.matins_weekday.{morn}.sessionals[{len(sets)}].items[0]'
        ...
        # reconstruct a full canon heading through its centered 'Tone VIII:'
        ...
        def full_heading(idx):
            ...
            while 'Tone VIII' not in txt and j + 1 < len(paras):
                nxt = paras[j + 1]['text']
            ...
            if t.startswith('Irmos:'):
                ...
                if is_incipit(body) and target == 2:
                    store[okey]['irmos'] = node(body, f'{DAYN[morn]} Matins, canon 2, Ode {ode} irmos (incipit device, §2.7)',
                        incipit_ref=f'tone8.matins_weekday.{morn}.canons[0].odes.{ode}.irmos')
                else:
...
paras1 = tokenize('/tmp/scan8/8-1L.txt')
node1 = make_node('8-1.pdf')
sc_i = hfind(paras1, 'COMPLINE')
try:
    noc_i = hfind(paras1, 'AT NOCTURNS')
except AssertionError:
    noc_i = hfind(paras1, 'AT NOCTURNES')  # tone-8 spelling
ci = sc_i + 1
...
# merge into tone3.js
src = open('/tmp/oh/src/data/octoechos_v2/tone8.js').read()
payload = src[src.index('export default ') + len('export default '):].rstrip().rstrip(';')
tone8 = json.loads(payload)
tone8['vespers_weekday'] = vespers_weekday
tone8['compline'] = compline
tone8['matins_weekday'] = matins_weekday
tone8['liturgy_weekday'] = liturgy_weekday
claims = [c for c in tone8['_encoded'] if '.' not in c]
...
tone8['_encoded'] = claims
header = src[:src.index('export default ') + len('export default ')]
open('/tmp/oh/src/data/octoechos_v2/tone8.js', 'w').write(header + json.dumps(tone8, ensure_ascii=False, indent=2) + ';\n')
import pickle
pickle.dump(CAPTURED_WK, open('/tmp/gen8/captured_t8_wk.pkl','wb'))
print('merged; nodes now:', json.dumps(tone8).count('"src"'))
