...
FILES = [
  {'f': '3-2', 'eve': 'sun', 'night': 'sun', 'morn': 'mon'},
  {'f': '3-3', 'eve': 'mon', 'night': 'mon', 'morn': 'tue'},
  {'f': '3-4', 'eve': 'tue', 'night': 'tue', 'morn': 'wed'},
  {'f': '3-5', 'eve': 'wed', 'night': 'wed', 'morn': 'thu'},
  {'f': '3-6', 'eve': 'thu', 'night': 'thu', 'morn': 'fri'},
  {'f': '3-7', 'eve': 'fri', 'night': 'fri', 'morn': 'sat'},
]
...
DIVERGENT_SHARED = {
  ('fri', 'vespers_pair'): 'departed period variance',
  ('fri', 'matins_pair'): "3-6 prints 'works of our hand' (singular)",
  ('sat', 'matins_trio'): "v1 'they' (T2 sat: 'those'); v3 'from generation to' (T2: 'unto generation and')",
  ('fri', 'lit_alleluia'): "3-6 'ages,' vs shared 'ages;'",
}
...
def closer_node(node, paras, i, locus, heading):
    ...
    sm = None
    if paras[i]['text'].startswith('Spec. Mel.'):
        sm = re.sub(r'^Spec\. Mel\.: “(.+?)( \.\.\.)?”:$', r'\1', paras[i]['text']); i += 1
    t = paras[i]['text']; i += 1
...
for cfg in FILES:
    f = cfg['f']; fname = f + '.pdf'
    paras = tokenize(f'/tmp/scan/{f}L.txt')
    ...
    if paras[i]['text'].startswith('Spec. Mel.'):
        ...
        spec_mel = norm(re.sub(r'^Spec\. Mel\.: “(.+?)( \.\.\.)?”:$', r'\1', paras[i]['text'])); i += 1
    if eve != 'fri':
        ...
        for n3 in range(3):
            ...
            fb_items.append(node(paras[i]['text'], f'{DAYN[eve]}-evening Vespers, Menaion-fallback sticheron {n3+1}', label='plain')); i += 1
        assert paras[i]['text'].startswith('Glory ..., Both now ...'), (f, paras[i]['text'])
        ...
        while True:
            ...
            if t.startswith('Glory ..., Both now ...') or 'Dogmatic' in t: break
            body = t
            if is_incipit(body) and n_seen:
                oct_items.append(node(body, f'Friday-evening Vespers, LIC sticheron {n_seen+1} (incipit repeat of sticheron 1, §2.7)',
                                      label='plain', incipit_ref='tone3.vespers_weekday.fri.lic.octoechos[0]',
                                      provenance_note=set_theme))
            else:
    ...
    while True:
        t = paras[i]['text']
        if t.startswith('Glory ..., Both now ...'): break
    ...
    while paras[j]['kind'] == 'heading' and not paras[j]['text'].startswith('Spec. Mel.'):
        a6_rub += ' ' + paras[j]['text']; j += 1
    sm = None
    if paras[j]['text'].startswith('Spec. Mel.'):
        sm = re.sub(r'^Spec\. Mel\.: “(.+?)( \.\.\.)?”:$', r'\1', paras[j]['text']); j += 1
    ...
    if morn != 'sat':
        ...
        while len(sets) < 3:
            ...
            while True:
                ...
                if paras[i]['kind'] == 'heading':
                    if paras[i]['text'].startswith('Spec. Mel.'):
                        sm2 = re.sub(r'^Spec\. Mel\.: “(.+?)”:$', r'\1', paras[i]['text']); i += 1; continue
                    i += 1; continue
                lab, text, printed = label_split(t)
                inc = is_incipit(text) and len(items) > 0
                kw = {}
                if inc:
                    kw['incipit_ref'] = f'tone3.matins_weekday.{morn}.sessionals[{len(sets)}].items[0]'
        ...
        for k9 in list(range(ode1_i + 1, ode1_i + 3)) + list(range(ode1_i - 1, max(0, ode1_i - 4), -1)):
            if paras[k9]['text'].startswith(('Canon of', 'Canon to')):
        ...
        while k2 < post_i:
            ...
            if t.startswith(('Canon of', 'Canon to')):
            ...
            if t.startswith('Irmos:'):
                ...
                if is_incipit(body) and target == 2:
                    store[okey]['irmos'] = node(body, f'{DAYN[morn]} Matins, canon 2, Ode {ode} irmos (incipit device, §2.7)',
                        incipit_ref=f'tone3.matins_weekday.{morn}.canons[0].odes.{ode}.irmos')
                else:
        ...
        for sn in range(2):
            ...
            while True:
                ...
                if t.startswith('Spec. Mel.'):
                    pend_sm = re.sub(r'^Spec\. Mel\.: “(.+?)( \.\.\.)?”:$', r'\1', t); i += 1; continue
                lab, text, printed = label_split(t)
        ...
        while True:
            ...
            if t.startswith('Glory ..., Both now'):
                ...
                pr_theo = node(text2, 'Saturday Matins, Praises Glory/Both-now Theotokion (§9.12 sic "Even who gave birth" kept verbatim)',
        ...
        sm5 = None
        if paras[k5]['text'].startswith('Spec. Mel.'):
            sm5 = re.sub(r'^Spec\. Mel\.: “(.+?)( \.\.\.)?”:$', r'\1', paras[k5]['text']); k5 += 1
        items3 = []
        i = k5
        while True:
            t = paras[i]['text']
...
paras1 = tokenize('/tmp/scan/3-1L.txt')
node1 = make_node('3-1.pdf')
...
# merge into tone3.js
src = open('/tmp/oh2/src/data/octoechos_v2/tone3.js').read()
payload = src[src.index('export default ') + len('export default '):].rstrip().rstrip(';')
tone3 = json.loads(payload)
tone3['vespers_weekday'] = vespers_weekday
tone3['compline'] = compline
tone3['matins_weekday'] = matins_weekday
tone3['liturgy_weekday'] = liturgy_weekday
claims = [c for c in tone3['_encoded'] if '.' not in c]
...
tone3['_encoded'] = claims
header = src[:src.index('export default ') + len('export default ')]
open('/tmp/oh2/src/data/octoechos_v2/tone3.js', 'w').write(header + json.dumps(tone3, ensure_ascii=False, indent=2) + ';\n')
import pickle
pickle.dump(CAPTURED_WK, open('/tmp/gen/captured_t3_wk.pkl','wb'))
print('merged; nodes now:', json.dumps(tone3).count('"src"'))
